            ...
            switch (request.params.name) {
                ...
                case "visit_page": {
                    ...
                    if (!isValidUrl(args.url)) {
                        return createErrorResponse(new McpError(MCP_ERRORS.InvalidParams, `Invalid URL: ${args.url}. Only http and https protocols are supported.`));
                    }
                    const visitArgs = {
                        url: args.url,
                ...
                case "take_screenshot": {
                    try {
                        ...
                        return createSuccessResponse(`Screenshot taken successfully. You can view it via *MCP Resources* (Paperclip icon) @ URI: ${result}`);
                    }
                    catch (error) {
                        return createErrorResponse(error);
                    }
                ...
                default:
                    return createErrorResponse(new McpError(MCP_ERRORS.MethodNotFound, `Unknown tool: ${request.params.name}`));
            }
        }
        catch (error) {
            return createErrorResponse(error);
